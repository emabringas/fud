...
NucDistance ResisBasedAntiviral::applies(const PseudoNucSequence& sequence) const
{
    ...
    std::string name;
//    this->get_attrib("name",name);
//    std::cout<< std::endl << "Antiviral : " << name << std::endl << std::endl;
    while (!it.end())
    {
//         std::cout << "Aminoacido : " << it -> aminoacids << std::endl;
//         std::cout << "Pos : " <<it -> pos << std::endl;
        TripletVal triplets_val = it->min_dist_to_resistance(sequence);
        min_dist = minimize(min_dist, triplets_val.second);

//         std::cout << "Distancia : " << min_dist << std::endl;
//     std::cout << "====================================" << std::endl;
//
